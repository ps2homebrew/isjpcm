...
#ifndef NOPRINT
		M_PRINTF("Memory Allocated. %ld bytes left.\n", QueryTotalFreeMemSize());
#endif
...
void SjPCM_PlayThread(void *param)
{
	...
	while (1) {
		...
		readpos += 512;
		if (readpos >= (BLOCKSIZE * 2 * numblocks))
			readpos = 0;

		{
			unsigned int rp = readpos, wp = writepos;
			if (wp < rp)
				wp += BLOCKSIZE * 2 * numblocks;
...
void *SjPCM_Enqueue(unsigned int *sbuff)
{
	...
	if (writepos >= (BLOCKSIZE * 2 * numblocks))
		writepos = 0;

	if (SyncFlag)
		if (writepos == (BLOCKSIZE * numblocks))
			readpos = 0x2400;
